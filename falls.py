...
DRAINAGE_PATH_GRASS = os.path.join(data, r'all_outlet_basins.gpkg') # GRASS delineated basins
DRAINAGE_PATH_SSTAT = os.path.join(data, r'all_outletbasins_ss.gpkg')  # USGS streamstats basins
...
## Save original cols
paired_df_cols = list(paired_df)
## Save new drainage area column names 
drainage_area_cols = []
...
for DRAINAGE_PATH in [DRAINAGE_PATH_GRASS, DRAINAGE_PATH_SSTAT]:
    ...
    drainage_df = paired_df.merge(drainages, how='inner', 
    ...
    DRAIN_AREA_version = '{}_{}'.format(DRAIN_AREA, version)
    drainage_area_cols.append(DRAIN_AREA_version)
    drainage_df[DRAIN_AREA_version] = drainage_df[DRAINAGE].apply(lambda x: x.area*1e-6)
    paired_df = paired_df.merge(drainage_df[['Name', DRAIN_AREA_version]], 
                                how='left', left_on='name', right_on='Name')
    paired_df_cols.append(DRAIN_AREA_version)
    paired_df = paired_df[paired_df_cols]
# Compute difference in drainage area sizes from the two methods
paired_df['Drainage Area Difference'] = paired_df[drainage_area_cols[1]] - paired_df[drainage_area_cols[0]]


## Solve for stream power equation - m:
## coeffiicient setting power law scaling between retreat rate and drainage area
paired_df[m_GRASS] = paired_df.apply(lambda x: stream_power_m(x[RETREAT], x[drainage_area_cols[0]]), axis=1)
paired_df[m_SS] = paired_df.apply(lambda x: stream_power_m(x[RETREAT], x[drainage_area_cols[1]]), axis=1)
m_GRASS_std = paired_df[m_GRASS].std()
m_GRASS_mean = paired_df[m_GRASS].mean()
m_SS_std = paired_df[m_SS].std()
m_SS_mean = paired_df[m_SS].mean()
...
if version == GRASS:
    col_num = 0
    c = 'b'
elif version == STREAM_STAT:
    col_num = 1
    c = 'r'
## Scatter of drainage area vs retreat rate
fig, ax = plt.subplots(1,1, figsize=(8,8))
# for i, row in paired_df.iterrows():
#     ax.plot([row[drainage_area_cols[0]], row[drainage_area_cols[1]]], [row[RETREAT], row[RETREAT]],
#             linestyle='dotted', color='grey')
# paired_df.plot.scatter(x=drainage_area_cols[col_num], y=RETREAT, s=35, c='b', ax=ax)
paired_df.plot.scatter(x=drainage_area_cols[col_num], y=RETREAT, s=35, c=c, ax=ax)

## Fit Polynomial
# Grass
z = np.polyfit(paired_df[drainage_area_cols[col_num]], paired_df[RETREAT], 2)
f = np.poly1d(z)
GRASS_DA_fit = np.linspace(paired_df[drainage_area_cols[col_num]].min(),
                            paired_df[drainage_area_cols[col_num]].max(),
                            1000)
GRASS_RET_fit = f(GRASS_DA_fit)
ax.plot(GRASS_DA_fit, GRASS_RET_fit, c=c)
# StreamStat
ss = paired_df.dropna()
z = np.polyfit(ss[drainage_area_cols[col_num]], ss[RETREAT], 2)
f = np.poly1d(z)
SS_DA_fit = np.linspace(paired_df[drainage_area_cols[col_num]].min(),
                            paired_df[drainage_area_cols[col_num]].max(),
                            1000)
SS_RET_fit = f(SS_DA_fit)
ax.plot(SS_DA_fit, SS_RET_fit)

# R-square
# fit values, and mean
x = paired_df.dropna()[drainage_area_cols[col_num]]
y = paired_df.dropna()[RETREAT]
yhat = f                      # or [p(z) for z in x]
...
## Fit Linear
# lm_GRASS = linregress(paired_df.dropna()[drainage_area_cols[col_num]], paired_df.dropna()[RETREAT])
# # lm_STSTA = linregress(paired_df.dropna()[drainage_area_cols[col_num]], paired_df.dropna()[RETREAT])
# if version == GRASS:
#     lm = lm_GRASS
# elif version == STREAM_STAT:
#     lm = lm_STSTA
    
# ax.plot(paired_df[drainage_area_cols[col_num]], lm_GRASS.slope*paired_df[drainage_area_cols[col_num]] + lm_GRASS.intercept, color=c)
# ax.plot(paired_df[drainage_area_cols[col_num]], lm_STSTA.slope*paired_df[drainage_area_cols[col_num]] + lm_STSTA.intercept, color='r')
...
#               fontsize=10,
#               )
label_point(paired_df[drainage_area_cols[col_num]], 
            paired_df[RETREAT],
            paired_df[NAME], ax=ax)
plt.tight_layout()

## Scatter m-values using GRASS and Stream Stat Drainages
m_fig, m_ax = plt.subplots(1,1,figsize=(8,8))
paired_df.plot.scatter(x=m_GRASS, y=m_SS, ax=m_ax)
label_point(paired_df[m_GRASS], paired_df[m_SS], paired_df[NAME], ax=m_ax)
...
paired_df.rename(columns={'name':'Falls', 
                          'ret_distance':'Retreat Distance (m)',
                          'incis_date_yrs_bp':'Time Incising (thousands years BP)',
                          'retreat_rate':'Retreat Rate (m / 1000 years)',
                          'drainage_area_km_GRASS':'Drainage Area GRASS (km^2)',
                          'drainage_area_km_STREAM_STAT':'Drainage Area STREAM STAT (km^2)',
                          m_GRASS: 'Power Law Coefficient - Using GRASS',
                          m_SS: 'Power Law Coefficient - Using Stream Stats'},
                  inplace=True)
paired_df['Falls'] = paired_df['Falls'].apply(lambda x: x.title())
paired_df.round(decimals=2).to_excel(os.path.join(PRJ_DIR, 'both_drainage_methods.xlsx'))